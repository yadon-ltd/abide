...
/*
  ...
  ── Logo edge fade ────────────────────────────────────────────
  LOGO_FADE_EDGES = true in config.php adds the 'logo-fade' CSS
  class to any <img class="header-logo"> element. The CSS mask
  rule in style.css fades the image edges into the background.
  Works with any image format — no transparent asset required.
  Set to false (default) when your logo has a transparent backing
  and you want precise edge control.
...
// Logo fade — adds 'logo-fade' class to <img> when LOGO_FADE_EDGES = true.
// The CSS mask rule in style.css handles the visual effect.
$_logo_fade  = defined('LOGO_FADE_EDGES') && LOGO_FADE_EDGES;
$_logo_class = 'header-logo' . ($_logo_fade ? ' logo-fade' : '');
...
<header>
  ...
  /*
    ...
    The logo-fade class is added when LOGO_FADE_EDGES = true.
  */
  ?>
    ...
    <div class="header-brand logo-only">
      <a href="/" class="brand-link" aria-label="<?php echo htmlspecialchars($_site_name); ?> — home">
        <img
          ...
          class="<?php echo htmlspecialchars($_logo_class); ?>"
        />
      </a>
    </div>
