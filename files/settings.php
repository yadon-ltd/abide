...
// ── Index page identity helpers ───────────────────────────────
/*
  The home page (index.php) has its own independent branding
  settings, separate from the header. This allows a different
  logo treatment on the landing page vs tool pages.

  Fallback chain:
    index-specific DB value → shared header DB value → constant → hard default

  Default logo mode for the index is 'both' (logo + site name),
  which is the most appropriate treatment for a landing page.
*/

/**
 * Resolved index logo mode.
 * DB: site.index_logo_mode  →  default: 'both'
 */
function abide_index_logo_mode(): string {
    $db = settings_get('site.index_logo_mode');
    if ($db !== '') return $db;
    return 'both';
}

/**
 * Resolved index logo file path.
 * DB: site.index_logo_file  →  site.logo_file (header)  →  constant: LOGO_FILE  →  '/assets/img/logo.png'
 */
function abide_index_logo_file(): string {
    $db = settings_get('site.index_logo_file');
    if ($db !== '') return $db;
    // Fall back to the header logo file
    return abide_logo_file();
}

/**
 * Resolved index logo alt text.
 * DB: site.index_logo_alt  →  site.logo_alt (header)  →  abide_site_name()
 */
function abide_index_logo_alt(): string {
    $db = settings_get('site.index_logo_alt');
    if ($db !== '') return $db;
    // Fall back to the header logo alt
    return abide_logo_alt();
}
