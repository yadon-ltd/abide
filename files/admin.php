...
// Index page identity defaults — independent from header branding
$index_identity_defaults = [
    'site.index_logo_mode' => abide_index_logo_mode(),
    'site.index_logo_file' => abide_index_logo_file(),
    'site.index_logo_alt'  => abide_index_logo_alt(),
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $allowed_keys = array_merge(
        ...
        array_keys($index_identity_defaults)
    );
...
<div class="admin-wrap">
  ...
  <form method="post" action="/admin">
    ...
    <!-- ── Section: Home Page Identity ─────────────────────── -->
    <div class="admin-section">

      <div class="admin-section-title">Home Page</div>
      <p class="admin-section-desc">
        Branding for the landing page (<code>index.php</code>), independent
        from the header. Defaults to <strong>logo + site name</strong> if not
        set. Logo file and alt text fall back to the header values above if
        left blank.
      </p>
      ...
      <div class="field-row">
        <div class="field">
          <label for="site_index_logo_mode">Logo mode</label>
          <select id="site_index_logo_mode" name="site_index_logo_mode">
            <?php
            $idx_mode_current = admin_val('site.index_logo_mode', $current, $index_identity_defaults);
            $idx_modes = [
                'both'       => 'Logo + site name (default)',
                'logo'       => 'Logo image only',
                'wordmark'   => 'Site name text only',
                'page_title' => 'Plain text (no logo)',
            ];
            foreach ($idx_modes as $val => $label) {
                $sel = ($idx_mode_current === $val) ? ' selected' : '';
                echo '<option value="' . $val . '"' . $sel . '>'
                   . htmlspecialchars($label)
                   . '</option>';
            }
      ...
      <div class="field-row">
        <div class="field">
          <label for="site_index_logo_file">Logo file path</label>
          <input type="text"
                 id="site_index_logo_file"
                 name="site_index_logo_file"
                 value="<?php echo htmlspecialchars(admin_val('site.index_logo_file', $current, $index_identity_defaults)); ?>"
                 placeholder="<?php echo htmlspecialchars(abide_logo_file()); ?>" />
          <span class="hint">leave blank to use header logo file</span>
        </div>
        <div class="field">
          <label for="site_index_logo_alt">Logo alt text</label>
          <input type="text"
                 id="site_index_logo_alt"
                 name="site_index_logo_alt"
                 value="<?php echo htmlspecialchars(admin_val('site.index_logo_alt', $current, $index_identity_defaults)); ?>"
                 placeholder="<?php echo htmlspecialchars(abide_logo_alt()); ?>" />
          <span class="hint">leave blank to use header alt text</span>
        </div>
      </div>

    </div><!-- /.admin-section home page identity -->
